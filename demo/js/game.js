...
class Player extends Raycaster.PlanarObject {
  constructor(raycaster,game,x,y,options={}) {
    ...
    const [w, h] = [125, 125];
    const padding = 10;
    this.minimap = new Raycaster.Minimap(this, .84, () => raycaster.instanceWidth * .03, () => raycaster.instanceWidth*.13, () => raycaster.instanceWidth*.13, undefined, undefined, {
      borderWidth: 3,
      relative: true
    });
...
let mainState = function(raycaster) {
  ...
  function preload() {
    ...
    raycaster.loadTexture('foo','images/test.gif',{alpha:true}).then((textureData) => {
    ...
    raycaster.loadTexture('backPlayer','images/player2.png');
  ...
  async function create() {
    map = await Raycaster.MapBuilder.buildFromFile(
      raycaster,
      'maps/map1.json',
      {
        0: null,
    ...
    let player1 = new Player(raycaster,game1,50,50,{color:new Raycaster.Color(255, 0, 0, 1)});
    // player1.setupMouse();
    // player1.mouse.start();
    let player2 = new Player(raycaster,game2,50,1);
    players.push(...[player1, player2]);

    raycaster.addGameObjects(players);

    raycaster.debugObjects.push(...players);
...
// TODO: add state management for menus, etc
